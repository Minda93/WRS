...
constexpr uint8_t RST_PIN = 53;     // Configurable, see typical pin layout above
constexpr uint8_t SS_1_PIN = 49;   // Configurable, take a unused pin, only HIGH/LOW required, must be diffrent to SS 2
...
constexpr uint8_t REPEAT_NUM = 5;
...
{ 
  for (uint8_t reader = 0; reader < NR_OF_READERS; reader++) {
    if (mfrc522[reader].PICC_IsNewCardPresent() && mfrc522[reader].PICC_ReadCardSerial()) {
      ...
      for(int8_t repeat = 0; repeat < REPEAT_NUM; repeat++){
        ...
        pub_topic(id, "999", 128, 198, 44, 164);
        pub_topic(id, "99999", 89, 10, 232, 43);
      }
    }
  }
  nh.spinOnce();

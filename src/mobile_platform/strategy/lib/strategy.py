        ...
        if(self._param.behavior == MOBILE_ROBOT):
            ...
            if(self._param.scanState):
                ...
                if(count):
                    ...
                    if(count <= math.ceil((scanNum)*(1./3))):
                        ...
                        # Method 1
                        # # 可視範圍 約 53 ~ -53 
                        # angle = (self._param.dis/float(scanNum)) / 53.0

                        # # self._dis = self._dis/5
                        # self.prevIntegral += self._param.dis
                        # derivative = self._param.dis - self.lastError
                        # self.lastError = self._param.dis
                        # turn = self._kp*self._param.dis+self._ki*self.prevIntegral+self._kd*derivative
                        # turn = turn/10000.0

                        # x = (self._param.minVel - abs(turn))*math.cos(math.radians(angle)) - turn*math.sin(math.radians(angle))
                        # y = self._param.minVel*math.sin(math.radians(angle)) + turn*math.cos(math.radians(angle))
                        # yaw = angle/53.0
                        # yaw = 0
                        ...
                        # x = (self._param.minVel - abs(turn))*math.cos(math.radians(self._param.ang)) - turn*math.sin(math.radians(self._param.ang))
                        # y = self._param.minVel*math.sin(math.radians(self._param.ang)) + turn*math.cos(math.radians(self._param.ang))

                        # yaw = 8*((self._param.ang/20.0)/math.sqrt(1+pow(self._param.ang/20.0,2.0)))
                        ...
                        # x = math.sqrt(self._param.minVel*self._param.minVel-y*y)
                        # x = self._param.minVel-abs(y)
                        x = (self._param.minVel - abs(y))*math.cos(math.radians(self._param.ang)) - y*math.sin(math.radians(self._param.ang))
                        # x = math.sqrt(self._param.minVel*self._param.minVel-y*y)*math.cos(math.radians(self._param.ang)) - y*math.sin(math.radians(self._param.ang))
            ...
            if(self._param.dis < 600):
                if(self._param.qrang is not None and self._param.qrang != 999):
                    if(abs(self._param.qrang) > self._param.errorAng):
                        if(self._param.qrang > 0):
                            x = 0
                            # y = 0
                            yaw = -self._param.velYaw
                        else:
                        ...
                        # self.Robot_Vel([y,-x,yaw])
                        self.Robot_Vel([x,y,yaw])
                        print('CORRECTION','angle',self._param.qrang)
                    else:
                        self.Robot_Stop()
                        self._param.behavior = PLATFORM
                        self.initPID = 1
